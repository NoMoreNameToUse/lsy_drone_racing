"""Model Predictive Contouring Control (MPCC) controller for the modular planning pipeline.

This is the last controller we implemented as we also want to try it out. We didn't focus
on MPCC as so many others are already doing MPCC and we want to explore a different approach

Also validated in real, though slower becoause of no real tuning.

How MPCC works is quite interesting though

Instead of tracking a time-parameterized reference, MPCC follows a geometric path
and treats progress along it as a free optimization variable, trading contour/lag
tracking error against a progress-speed reward directly inside the acados NLP.
...
from lsy_drone_racing.control.controllers.modules.path_generator_mpcc import (
    PathConfig,
    PathPlanner,
    TrajectoryResult,
)
...
class MPCCController(Controller):
    ...
    def __init__(
        ...
        # Vertical takeoff target, held until the vehicle settles (see _finish_warmup).
        ...
        # Sensor-discovery flags used to trigger replanning.
        ...
        # Control state carried between steps (last commands and path progress).
        self.last_theta = 0.0
    ...
    def _plan_trajectory(self, obs: dict[str, NDArray[np.floating]]):
        ...
        # Cached for the MPCC cost parameters (gate/obstacle proximity weights).
    ...
    def _build_solver(self):
        ...
        # State box constraints on [f, f_cmd, r_cmd, p_cmd, y_cmd].
        ...
        # Input box constraints on [df_cmd, dr_cmd, dp_cmd, dy_cmd, v_theta_cmd].
        thrust_rate = float(self.mpcc_cfg.max_thrust_rate)
        attitude_rate = float(self.mpcc_cfg.max_attitude_rate)
        ...
        ocp.constraints.x0 = np.zeros(self.nx)  # overwritten each step with the measured state
    ...
    def _build_dynamics_model(self) -> AcadosModel:
        ...
        # Attitude-rate model parameters derived from drone-models
    ...
    def _encode_trajectory_params(self) -> np.ndarray:
        ...
        # Dynamic contour/lag weights: raised near gates and obstacles.
        ...
        for obst_center in self._cached_obstacles:
            ...
            qc_dyn = np.maximum(qc_dyn, qc_obs)  # obstacle proximity
    ...
    def _build_cost_expression(self) -> MX:
        ...
        # Reference position, tangent and dynamic weight at the current progress theta.
        ...
        # Split the tracking error into lag (along path) and contour (perpendicular).
    ...
    def compute_control(
        ...
        One control tick is a small state machine: takeoff warmup, then (every
        tick) an environment-change replan check, theta/recovery bookkeeping, and
        either a recovery-mode command or a full MPCC solve. Each stage is a
        separate method below; this method only sequences them and owns the
        early-return points (kept here so the tick's control flow stays visible
        in one place).
        ...
        if not self._warmup_complete:
            warmup_cmd = self._advance_warmup(obs)
            if warmup_cmd is not None:
                return warmup_cmd
            # Warmup finished this tick: fall through into a full MPCC step below.

        self._replan_if_environment_changed(obs)
        ...
        x_now = self._current_state_vector(obs)
        self._seed_solver(x_now)
        self._check_termination(obs)

        status = self.solver.solve()
        if status != 0:
            return self._handle_solver_failure(obs, status)
        return self._finish_successful_step()

    def _advance_warmup(self, obs: dict[str, NDArray[np.floating]]) -> NDArray[np.floating] | None:
        """Advance takeoff warmup; return a command while warming up, else None.

        Returning ``None`` signals ``compute_control`` to fall through into a full
        MPCC step on the same tick the warmup completes, matching the original
    ...
    def _replan_if_environment_changed(self, obs: dict[str, NDArray[np.floating]]) -> None:
        """Replan the trajectory and reset solver/theta state if gates/obstacles moved."""
        if not self._detect_environment_change(obs):
            return
    ...
    def _current_state_vector(self, obs: dict[str, NDArray[np.floating]]) -> NDArray[np.floating]:
        """Assemble the augmented MPCC state: pos, vel, rpy, thrust/attitude cmds, theta."""
        quat = obs["quat"]
    ...
    def _seed_solver(self, x_now: NDArray[np.floating]) -> None:
        """Shift (or init) the warm start, load it into the solver, and fix the initial state."""
    ...
    def _check_termination(self, obs: dict[str, NDArray[np.floating]]) -> None:
        """Set ``self.finished`` once all gates are passed or the drone leaves safe bounds."""
    ...
    def _handle_solver_failure(
        self, obs: dict[str, NDArray[np.floating]], status: int
    ) -> NDArray[np.floating]:
        """After a failed solve: enter recovery once failures persist, else hold last command."""
    ...
    def _finish_successful_step(self) -> NDArray[np.floating]:
        """Extract the MPC solution, update last commands, log periodically, return the command."""
        self._solver_failure_count = 0
    ...
    def get_debug_lines(
        ...
        """Return (points, rgba, start_size, end_size) line segments for visualization.

        Each segment is independently wrapped in a broad ``except`` and simply
        omitted on failure: this is visualization-only, built from the same state
        the control loop already uses safely, and must never raise back into
        ``render_callback``/the sim loop over a transient/edge-case shape mismatch.
        """
        debug_lines = []
