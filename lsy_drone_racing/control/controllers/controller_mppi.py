...
from lsy_drone_racing.control.controllers.modules.path_generator import (
    GatePassingPathGenerator,
    ThetaStarGatePathGenerator,
)
from lsy_drone_racing.control.controllers.modules.timing_module import MotionAwareTiming
    ...
    def _rollout_cost(
        ...
        for i in range(self.cfg.horizon):
            ...
            if obstacles.size:
                ...
                cost += 1000.0 * np.sum(intrusion**2, axis=1)
            ...
            if gate_position is not None and gate_rotation is not None:
                ...
                plane_weight = np.exp(-0.5 * (local[:, 0] / 0.35) ** 2)
