...
class CompositeSpline:
    ...
    def __init__(self, first: CubicSpline, second: CubicSpline, offset: float):
        """Join two splines, with ``second`` starting at ``offset`` on the shared axis."""
    ...
    def __call__(self, t: float | NDArray[np.floating]) -> NDArray[np.floating]:
        """Evaluate the composite spline at ``t`` (scalar or array)."""
        if np.isscalar(t):
            return self.trajectory_1(t) if t < self.offset else self.trajectory_2(t - self.offset)
        return np.array([self(t_i) for t_i in t])

    def derivative(self, order: int = 1) -> CompositeSpline:
        """Return the ``order``-th derivative as another ``CompositeSpline``."""
        return CompositeSpline(
            self.trajectory_1.derivative(order), self.trajectory_2.derivative(order), self.offset
        )
...
class PathPlanner:
    ...
    def add_detour_waypoints(
        ...
        for i in range(num_gates - 1):
            ...
            if last_idx_gate_i >= len(waypoints_list) or first_idx_gate_i_plus_1 >= len(
                waypoints_list
            ):
                break
            ...
            if angle_deg > angle_thresh:
                ...
                self._debug_info["detour_analysis"].append(
                    {"gate_index": i, "angle": angle_deg, "detour_waypoint": detour_waypoint.copy()}
                )
        ...
        for obstacle_pos in obstacle_positions:
            ...
            for i, point in enumerate(trajectory_points):
                ...
                if distance_xy < safe_dist:
                    ...
                    collision_free_times.append(
                        (time_samples[entry_index] + time_samples[exit_index]) / 2
                    )
    ...
    def create_spline(self, duration: float, waypoints: NDArray[np.floating]) -> CubicSpline:
    ...
    def extend_spline(
        self, trajectory: CubicSpline, extend_length: Optional[float] = None
    ) -> CubicSpline:
        ...
        extra_knots = np.arange(base_knots[-1] + base_dt, base_knots[-1] + ext_len, base_dt)
    ...
    def compute_curvature(
        self, spline: CubicSpline, t_vals: NDArray[np.floating], eps: float = 1e-8
    ) -> NDArray[np.floating]:
    ...
    def find_closest_point(
        self, trajectory: CubicSpline, position: NDArray[np.floating], sample_interval: float = 0.05
    ...
    def plan_trajectory(
        ...
        waypoints = self.generate_waypoints(initial_pos, gate_positions, gate_normals)

        # Step 2: Add detour waypoints for backtracking gates
        waypoints = self.add_detour_waypoints(
            waypoints, gate_positions, gate_normals, gate_y_axes, gate_z_axes
        )
