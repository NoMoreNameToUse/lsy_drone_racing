"""Dynamics-aware timing for the modular planning pipeline."""
...
class DynamicTiming:
    """Feasible, clearance/curvature-shaped per-waypoint speed profile.

    Assigns per-waypoint timestamps from a speed profile that:

    * **slows for geometry** -- per-waypoint curvature (turn angle / segment
      length) caps speed at ``sqrt(a_lat_max / kappa)``, so tight corners and
      reversals are taken slowly;
    * **slows for tightness** -- the clearance "tube" from the post-processor
      scales speed down in narrow passages / squeezes between obstacles;
    * **handles reversals** -- a near-180 deg fold is caught by the curvature term
      and hard-capped at ``reversal_speed``; the backward acceleration pass makes
      the drone decelerate *before* reaching it, while still allowing full speed
      through an open straight gate;
    * **stays feasible** -- a forward/backward pass limits tangential
    ...
    def _accel_limited_pass(
        self, v_lim: NDArray[np.floating], ds: NDArray[np.floating], v_init: float | None = None
    ) -> NDArray[np.floating]:
    ...
    def _integrate(self, v: NDArray[np.floating], ds: NDArray[np.floating]) -> NDArray[np.floating]:
