...
from lsy_drone_racing.control.controllers.modules.trajectory_module_improved import (  # noqa: E402
    SplineTrajectory,
)
...
def _race_env(cfg: object) -> JaxToNumpy:
...
def build_track_bank(
    ...
    for seed in range(n_tracks):
        ...
        if (seed + 1) % 500 == 0:
            print(
                f"  {seed + 1}/{n_tracks} tracks ({len(refs)} ok, {n_fail} failed, "
...
class TrackTrajEnv(RandTrajEnv):
    ...
    def __init__(self, bank_path: str | Path = BANK_PATH, **kwargs: Any):
...
def build_bank(
    ...
    return build_track_bank(
        ...
        n_tracks=n_tracks,
        out=out,
        v_max_range=(v_max_lo, v_max_hi),
        a_max_range=(a_max_lo, a_max_hi),
    )
...
def train(
    ...
    Saves to ``controllers/utility/ckpt/<out_ckpt>`` -- NOT the deployed
    ``control/ppo_drone_racing.ckpt`` -- so the working checkpoint is preserved
    until you validate this one on the track.
    ...
    ckpt_dir = REPO_ROOT / "lsy_drone_racing" / "control" / "controllers" / "utility" / "ckpt"
    ckpt = ckpt_dir / out_ckpt

    def env_fn(
        num_envs: int, jax_device: str, torch_device: torch.device, coefs: dict
    ) -> VectorEnv:
