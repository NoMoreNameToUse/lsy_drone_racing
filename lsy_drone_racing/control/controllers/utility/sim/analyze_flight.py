...
def build_report(flight: dict, m: dict, events: list[dict], p: dict) -> str:
    ...
    lines.append(
        f"seed={flight['seed']}  reason={flight['reason']}  "
    ...
    lines.append(
        f"  max position error     : {m['pos_err'].max():.3f} m   (mean {m['pos_err'].mean():.3f})"
    )
    lines.append(f"  max speed              : {m['speed'].max():.3f} m/s")
    lines.append(f"  max speed error        : {np.nanmax(m['speed_err']):.3f} m/s")
    lines.append(
        f"  max reversal angle     : exec {m['turn_exec'].max():.0f} deg, "
...
def plot_overview(flight: dict, m: dict, events: list[dict], p: dict, out_png: Path) -> None:
    ...
    for e in events:
        ...
        seen.add(e["kind"])
        axm.scatter(e["pos"][0], e["pos"][1], c=color, marker=marker, s=size, label=lbl, zorder=5)
...
def plot_3d(flight: dict, m: dict, events: list[dict], out_html: Path) -> None:
    ...
    for kind, evs in by_kind.items():
        ...
        txt = [
            f"{kind}<br>t={e['t0']:.2f}s<br>value={e['value']:.3f}"
            f"{('<br>' + e['extra']) if e['extra'] else ''}"
            for e in evs
        ]
        traces.append(
            go.Scatter3d(
    ...
    fig.update_layout(
        title=(
            f"Flight 3D: seed {flight['seed']} ({flight['reason']})  "
...
def analyze(
    ...
    p = {
        ...
        "reversal_min_speed": reversal_min_speed,
        "pos_dev": pos_dev,
        "speed_dev": speed_dev,
    }
