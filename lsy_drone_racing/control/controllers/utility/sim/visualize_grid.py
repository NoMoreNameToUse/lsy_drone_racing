...
def reconstruct_scene(flight: dict, replan: int) -> dict:
    ...
    if replan >= n_replans:
        raise IndexError(
            f"replan {replan} out of range (log has {n_replans} replans; "
...
def build_grids(
    scene: dict, config: object, resolution: float, safety_margin: float, obstacle_radius: float
) -> dict:
    ...
    full = OccupancyGrid3D(
        obs={
            "gates_pos": scene["gates_pos"],
            "gates_quat": scene["gates_quat"],
            "obstacles_pos": scene["obstacles_pos"],
        },
        **common,
    )
    obst_only = OccupancyGrid3D(
        obs={
            "gates_pos": _empty(3),
            "gates_quat": _empty(4),
            "obstacles_pos": scene["obstacles_pos"],
...
def plot_grid_3d(
    flight: dict, scene: dict, grids: dict, out_html: Path, voxel_size: float, max_voxels: int
) -> None:
    ...
    for key, color, name in (
        ("obstacle", "saddlebrown", "occupied: obstacle"),
    ...
    for g in range(gpos.shape[0]):
        for half, c, w, nm in (
            (GATE_APERTURE_HALF, "deepskyblue", 6, "gate aperture"),
    ...
    fig.update_layout(
        title=(
            f"A* occupancy grid: seed {flight['seed']} | {scene['label']} | "
...
def visualize(
    ...
    if list_replans:
        print(
            f"{path.name}: {int(flight['n_replans'])} replans "
            f"(use --replan -1 for the initial plan)"
        )
        for k in range(int(flight["n_replans"])):
            print(
                f"  replan {k}: global_tick={int(flight['replan_ticks'][k])}, "
    ...
    print(
        f"Grid: shape={tuple(int(v) for v in grids['full'].shape)}, "
