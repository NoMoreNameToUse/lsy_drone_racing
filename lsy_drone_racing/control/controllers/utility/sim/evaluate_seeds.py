    ...
    *,
    ...
    if terminated:
        ...
        if below.any() or above.any():
            axis = "xyz"
            viol = [f"{axis[i]}={last_pos[i]:.2f}" for i in range(3) if below[i] or above[i]]
...
def _build_report(results: list[dict], config_label: str) -> str:
    ...
    lines.append(
        f"Median speed     : {_median(pass_speeds)} m/s (passed runs)   "
    ...
    if failed:
        ...
        lines.append("")
        lines.append(
            f"Not-passed gate histogram (gate -> #fails): "
