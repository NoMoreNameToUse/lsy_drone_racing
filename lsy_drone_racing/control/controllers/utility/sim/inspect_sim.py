...
def clipping_report(
    track: dict, obstacle_radius: float = 0.18, drone_margin: float = 0.10
) -> tuple[str, bool]:
...
def _inspect_loop(env: DroneRaceEnv, controller: Controller | None, render: bool) -> str:
...
def _fly(
    env: DroneRaceEnv, controller: Controller, config: ConfigDict, obs: dict, info: dict
) -> float | None:
