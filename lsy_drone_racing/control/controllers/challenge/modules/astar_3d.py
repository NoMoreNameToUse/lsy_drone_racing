"""Pure-Python 26/18-connected 3D A* for the initial-challenge grid planner."""
...
def astar_3d(
    ...
    preferred_direction: NDArray[np.floating] | None = None,
    direction_bias_weight: float = 1.0,
    direction_bias_decay: float = 8.0,
    min_direction_speed: float = 0.10,
) -> list[tuple[int, int, int]] | None:
    """Basic 18-connected 3D A* on an occupancy grid.

    Args:
        grid: Occupancy grid exposing ``is_free``/``occupied``/``shape``.
        start_idx: Start grid index (must be free).
        goal_idx: Goal grid index (must be free).
        max_iterations: Cap on expansions before giving up.
...
def heuristic(a: tuple[int, int, int], b: tuple[int, int, int]) -> float:
    """Euclidean distance between two grid indices."""
    dx = a[0] - b[0]
    dy = a[1] - b[1]
    dz = a[2] - b[2]
...
def reconstruct_path(
    parents: np.ndarray, start_idx: tuple[int, int, int], goal_idx: tuple[int, int, int]
) -> list[tuple[int, int, int]] | None:
    """Walk the ``parents`` array back from goal to start to recover the index path."""
    current = goal_idx
    path = [current]
