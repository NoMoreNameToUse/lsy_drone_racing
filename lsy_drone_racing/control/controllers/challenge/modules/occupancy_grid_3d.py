"""3D occupancy grid for the initial-challenge grid-based path planners."""
...
class OccupancyGrid3D:
    ...
    def _build_occupancy_grid(self) -> None:
        """Mark obstacle poles and gate frames occupied."""
    ...
    def _inside_inflated_pole(self, p: NDArray[np.floating]) -> bool:
        """True if ``p`` is within an inflated obstacle pole (vertical, XY test)."""
        for obs_p in self.obstacles_pos:
    ...
    def _inside_gate_frame(self, p: NDArray[np.floating]) -> bool:
        """True if ``p`` is in an inflated gate frame (inner opening stays free).

        In gate-local axes x is the normal, y lateral and z vertical relative to the
        gate center; occupied if within the outer square frame but outside the
        shrunken inner opening.
        """
        for gate_pos, gate_quat in zip(self.gates_pos, self.gates_quat):
            rot = R.from_quat(gate_quat)
