"""Initial-challenge exploratory controller: A* pipeline + the example MPC tracker.

Modular pipeline (gate-anchored A* path generator on a 3D occupancy grid ->
distance-based timing -> spline trajectory) tracked by the course's example MPC,
with event-triggered replanning. First exploratory approach that grew into
``controller_mpc_astar``; kept so the initial-challenge submission stays runnable.
"""
...
class AttitudeMPC(Controller):
    ...
    def __init__(self, obs: dict[str, NDArray[np.floating]], info: dict, config: dict):
        ...
        self.path_gen = AStarGatePathGenerator(
            grid_resolution=0.05, safety_margin=0.05, obstacle_radius=0.215, prune_path=True
        )
        ##self.timing = UniformTiming()

        self.timing = DistanceTiming(nominal_speed=1, min_segment_time=0.1)
